...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Evaluation & Payment Processor Integration
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; Simple validation for payment processor integration
(define-read-only (validate-payment (agent-id principal) (recipient principal) (amount uint))
  (let ((max-id (var-get rule-counter)))
    (check-payment-rules agent-id recipient amount u1 max-id)))

(define-private (check-payment-rules (agent-id principal) (recipient principal) (amount uint) (current-id uint) (max-id uint))
  (if (> current-id max-id)
      (ok true) ;; No violations found
      (let ((rule (map-get? rules { rule-id: current-id })))
        (match rule
          r (if (and (is-eq (get agent-id r) agent-id) (get enabled r))
                (let ((rule-type (get rule-type r)))
                  (if (is-eq rule-type "amount")
                      (let ((params (map-get? rule-amount { rule-id: current-id })))
                        (match params p
                          (if (or (< amount (get min-amount p)) (> amount (get max-amount p)))
                              (err u400) ;; Amount rule violation
                              (check-payment-rules agent-id recipient amount (+ current-id u1) max-id))
                          (check-payment-rules agent-id recipient amount (+ current-id u1) max-id)))
                      (if (is-eq rule-type "merchant")
                          (let ((params (map-get? rule-merchant { rule-id: current-id })))
                            (match params p
                              (let ((in-whitelist (p-list-contains (get merchants p) recipient))
                                    (mode (get mode p)))
                                (if (and (is-eq mode "whitelist") (not in-whitelist))
                                    (err u401) ;; Recipient not whitelisted
                                    (if (and (is-eq mode "blacklist") in-whitelist)
                                        (err u402) ;; Recipient blacklisted
                                        (check-payment-rules agent-id recipient amount (+ current-id u1) max-id))))
                              (check-payment-rules agent-id recipient amount (+ current-id u1) max-id)))
                          (check-payment-rules agent-id recipient amount (+ current-id u1) max-id))))
                (check-payment-rules agent-id recipient amount (+ current-id u1) max-id))
          none (check-payment-rules agent-id recipient amount (+ current-id u1) max-id)))))

;; Get spending limits for payment processor
(define-read-only (get-spending-limits (agent-id principal))
  (let ((max-id (var-get rule-counter)))
    (find-spending-limits agent-id u1 max-id)))

(define-private (find-spending-limits (agent-id principal) (current-id uint) (max-id uint))
  (if (> current-id max-id)
      (ok { daily-limit: u1000000, monthly-limit: u10000000 }) ;; Default limits
      (let ((rule (map-get? rules { rule-id: current-id })))
        (match rule
          r (if (and (is-eq (get agent-id r) agent-id)
                    (get enabled r)
                    (is-eq (get rule-type r) "spending"))
                (let ((params (map-get? rule-spending { rule-id: current-id })))
                  (match params p
                    (ok { daily-limit: (get daily p), monthly-limit: (get monthly p) })
                    (find-spending-limits agent-id (+ current-id u1) max-id)))
                (find-spending-limits agent-id (+ current-id u1) max-id))
          none (find-spending-limits agent-id (+ current-id u1) max-id)))))

;; Check if recipient is whitelisted
(define-read-only (is-recipient-allowed (agent-id principal) (recipient principal))
  (let ((max-id (var-get rule-counter)))
    (check-recipient-rules agent-id recipient u1 max-id)))

(define-private (check-recipient-rules (agent-id principal) (recipient principal) (current-id uint) (max-id uint))
  (if (> current-id max-id)
      true ;; Default allow if no merchant rules found
      (let ((rule (map-get? rules { rule-id: current-id })))
        (match rule
          r (if (and (is-eq (get agent-id r) agent-id)
                    (get enabled r)
                    (is-eq (get rule-type r) "merchant"))
                (let ((params (map-get? rule-merchant { rule-id: current-id })))
                  (match params p
                    (let ((in-whitelist (p-list-contains (get merchants p) recipient))
                          (mode (get mode p)))
                      (if (is-eq mode "whitelist")
                          in-whitelist
                          (if (is-eq mode "blacklist")
                              (not in-whitelist)
                              (check-recipient-rules agent-id recipient (+ current-id u1) max-id))))
                    (check-recipient-rules agent-id recipient (+ current-id u1) max-id)))
                (check-recipient-rules agent-id recipient (+ current-id u1) max-id))
          none (check-recipient-rules agent-id recipient (+ current-id u1) max-id)))))
...
;; Helper function to create basic spending rule for an agent
(define-public (create-spending-rule (agent-id principal) (daily-limit uint) (monthly-limit uint))
  (let ((rule-id (try! (create-rule agent-id "spending" "Basic spending limits" "enforce"))))
    (try! (set-spending-params rule-id daily-limit u0 monthly-limit))
    (ok rule-id)))

;; Helper function to create basic merchant whitelist rule
(define-public (create-merchant-whitelist (agent-id principal) (allowed-merchants (list 50 principal)))
  (let ((rule-id (try! (create-rule agent-id "merchant" "Merchant whitelist" "deny"))))
    (try! (set-merchant-params rule-id "whitelist" allowed-merchants (list)))
    (ok rule-id))))
